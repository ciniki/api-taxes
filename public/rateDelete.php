...
// Description
// ===========
// This method will remove a tax rate from a tenant, but only if it's not
...
function ciniki_taxes_rateDelete(&$ciniki) {
    ...
    $rc = ciniki_taxes_checkAccess($ciniki, $args['tnid'], 'ciniki.taxes.rateDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    foreach($modules as $module => $m) {
        ...
        if( $rc['stat'] == 'ok' ) {
            $fn = $rc['function_call'];
            $rc = $fn($ciniki, $modules, $args['tnid'], 'ciniki.taxes.rate', $args['rate_id']);
            if( $rc['stat'] != 'ok' ) {
    ...
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.taxes.rate', 
